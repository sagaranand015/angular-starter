angular.module('monitorApp').controller('qaPieController', qaPieController);

qaPieController.$inject = [ '$scope', 'dataFactory', '$rootScope', 'ngToast',
		'utilityService', '$window', '$uibModal' ];

function qaPieController($scope, dataFactory, $rootScope, ngToast,
		utilityService, $window, $uibModal) {
	console.log("Logging qaPieController");
	var vm = this;
	...
	vm.initEncryptDecrypt = function initEncryptDecrypt() {
		...
		vm.prodMachines = dataFactory
				.getRequestData("resources/qa-servers.json")
				.then(
	 ...
	 */
	$scope.checkFesNodes = function checkFesNodes(url, endpoint) {
